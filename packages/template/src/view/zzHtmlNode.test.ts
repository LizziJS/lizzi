...
import { ReactiveValueView } from "./zzHtmlNode";
import { TextNodeView } from ".";
import { zzNode } from "@lizzi/node";

describe("ReactiveValueView", () => {
  it("should create an ReactiveValueView", () => {
    expect(ReactiveValueView).toBeInstanceOf(Function);
    const view = new ReactiveValueView({ children: new zzString("test") });

    expect(view).toBeInstanceOf(ReactiveValueView);
  });

  it("should use text node for zzString", () => {
    const string = new zzString("test");
    const view = new ReactiveValueView({ children: string });
  ...
  it("should use object node for zzObject", () => {
    ...
    const view = new ReactiveValueView({ children: object });
