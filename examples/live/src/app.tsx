import { zzIf, zzObject, zzString } from "@lizzi/core";
import { Body, onClick, ViewComponent, ViewNode } from "@lizzi/template";

import "./app.css";

class Example extends ViewComponent {
  constructor({ name, embed }: { name: string; embed: string }) {
    super();
    ...
    this.append(
      <a
        class={[
          ...
          zzIf(() => selected.value === embed, "bg-orange-300", ""),
        ]}
        use={[onClick(() => (selected.value = embed))]}
      >
        {name}
      </a>
    );
  ...
  constructor(embed: string) {
    ...
    this.append(
      <iframe
        src={
          embed +
          "?autoresize=1&expanddevtools=1&fontsize=14&hidenavigation=1&theme=dark&module=%2Fsrc%2Fapp.tsx"
        }
        style={{
          position: "relative",
          width: "100%",
...
const selected = new zzString();

const result = new zzObject<ExampleView>();

selected.onChange.addListener(() => {
  if (selected.value) {
    result.value = new ExampleView(selected.value);
  }
});
